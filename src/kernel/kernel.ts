...
export class Kernel extends TgdPainter {
  ...
  debugVariables() {
    const output: Record<string, Record<string, BasikValue>> = {};
    for (let i = 0; i < this.variablesStack.length; i++) {
      const variables = this.variablesStack[i];
      const routineName = this.callStack[i];
      const items: Record<string, BasikValue> = {};
      for (const key of variables.keys()) {
        const value = variables.get(key);
        if (value !== undefined) items[key] = value;
      }
      output[routineName] = items;
    }
    console.log(output);
    return output;
  }
  ...
  subroutineEnter(
    ...
    this.callStack.push(routineName);

...
export class BasikAssembly {
  ...
  async execute(code: string) {
    ...
    try {
      ...
      err.msg +=
        "\n\n" + JSON.stringify(this.kernel.debugVariables(), null, "  ");
      ...
      this.kernel.cursor.hide();
      this.kernel.onKeyPressed = null;
    }
  }
...
On ne peut donc pas lui assigner d'élément.`);
  ...
  readonly $setVars = makeAsync("setVar(name, value)", () => {
    ...
    assertType<string[]>(varNames, ["array", "string"]);
    if (!Array.isArray(varArray)) {
      this.lexer.fatal(`Je ne peux desctructurer que des listes.
Mais j'ai reçu $[${varNames.map((name) => name.slice(1)).join(", ")}] = ${JSON.stringify(varArray)}.`);
    }
  ...
  readonly $function = async () => {
    ...
    this.kernel.subroutineEnter(name, userFunc.args, args, null);
  ...
  readonly $procedure = async () => {
    ...
    if (!instructionExist) {
      ...
      this.kernel.subroutineEnter(name, userFunc.args, args, () =>
        this.stack.pop(),
      );
      this.callStack.push(this.cursor);
      this.cursor = userFunc.cursor;

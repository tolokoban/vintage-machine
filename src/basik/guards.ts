...
export function argsAreOneArrayOrOneString() {
    return (args: BasikValue[]): asserts args is [BasikValue[] | string] => {
        if (args.length !== 1) {
            throw new Error("Cette fonction attend un et un seul argument.")
        }
        const [arg] = args
        if (!isType(arg, ["|", "string", ["array", "unknown"]])) {
            throw new Error(
                "L'unique argument de cette fonction devrait être une liste ou un texte"
            )
        }
    }
}
